"""
This file compute the solution of Stochastic Inverse Problem of truncated domain,
and generate figures used in the dissertation
"""
...
if MC_assumption is False:
    input_samples.estimate_volume(n_mc_points=1E7)
else:
...
# Get index of high probability points
...
# Generate the first 10 curves corresponding to the high probability Fourier coefficients samples
sol_all = np.zeros((1001,10))

for i in range(0,10):
    sol = lr.linear_ode_fourier(t,input_samples._values_local[arghigh[i],0::2],
                                input_samples._values_local[arghigh[i],1::2])
    sol_all[:,i]=sol


# Generate the figures
# figure Figurelr is not used in the final dissertation
...
plt.savefig(figurepath+"/Figurelr.pdf")
...
#plt.ylim( (-0.2, 0.3) )
plt.savefig(figurepath+"/fundecay"+str(decaynum)+".pdf")
